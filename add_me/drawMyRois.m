  ...
  for pp=1:length(mgznames)
    if ischar(mgznames{pp})
      for hh=1:length(hemis)
        ...
        file0 = sprintf('%s/%s.%s.mgz',fsdir0,hemis{hh},mgznames{pp});
...
%       [~,Lookup{zz},~,himgs{zz}] = cvnlookup(subjid,viewmode,cat(1,[alldata{zz,1},alldata{zz,2}])',crngs{zz},cmaps{zz},threshs{zz},[],2);
      [~,Lookup{zz},~,himgs{zz}] = cvnlookup(subjid,viewmode,...
          [reshape(alldata{zz,1},numel(alldata{zz,1}),1);reshape(alldata{zz,2},...
          numel(alldata{zz,2}),1)],crngs{zz},cmaps{zz},threshs{zz},[],2,{'roiname',{'Kastner2015';' '},'roicolor',{'w';'k'},'drawroinames',0,'roiwidth',{3,10},'fontsize',20,'surfshading',false});
